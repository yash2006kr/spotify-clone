...
async function deleteCoverIfUnused(coverId: ObjectId) {
  ...
  try {
    ...
    // Missing GridFS files should not block artist cover updates.
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    await ensureIndexes();
    const db = await getDb();
    const existingTracks = await db.collection("tracks").find({ artist }).toArray();
    ...
    await db.collection("tracks").updateMany({ artist }, { $set: { coverId, updatedAt: new Date() } });

    await Promise.all([...previousCoverIds.values()].map((id) => deleteCoverIfUnused(id)));

    const updatedTracks = await db.collection("tracks").find({ artist }).sort({ createdAt: -1 }).toArray();
